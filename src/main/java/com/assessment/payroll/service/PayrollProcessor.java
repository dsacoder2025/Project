...
public class PayrollProcessor {
    ...
    public BigDecimal calculateTax(BigDecimal grossPay) {
        if (grossPay.compareTo(BigDecimal.ZERO) < 0) return BigDecimal.ZERO;
    ...
    public Map<String, BigDecimal> calculateDeductions(Employee employee, BigDecimal grossPay) {
         if (employee == null) throw new IllegalArgumentException("Employee cannot be null");
         Map<String, BigDecimal> rawDeductions = getStrategy(employee.getEmployeeType()).calculateDeductions(employee, grossPay);
    ...
    public List<PaySlip> processMonthlyPayroll(Map<Employee, BigDecimal> employeeWorkUnits) {
         if (employeeWorkUnits == null) throw new IllegalArgumentException("Input map cannot be null");
